 ...
 * =============================================================================
 *
 *       Filename:  leveldb_item.c
 *
 *    Description:  item structure used in leveldb engine implementation.
 *
...
static void item_link_q(struct leveldb_engine *engine, hash_item *it);
static void item_unlink_q(struct leveldb_engine *engine, hash_item *it);
static hash_item *do_item_alloc(struct leveldb_engine *engine,
                                const void *key, const size_t nkey,
                                const int flags, const rel_time_t exptime,
                                const int nbytes,
                                const void *cookie);
static hash_item *do_item_get(struct leveldb_engine *engine,
                              const char *key, const size_t nkey);
static int do_item_link(struct leveldb_engine *engine, hash_item *it);
static void do_item_unlink(struct leveldb_engine *engine, hash_item *it);
static void do_item_release(struct leveldb_engine *engine, hash_item *it);
static void do_item_update(struct leveldb_engine *engine, hash_item *it);
static int do_item_replace(struct leveldb_engine *engine,
                            hash_item *it, hash_item *new_it);
static void item_free(struct leveldb_engine *engine, hash_item *it);
...
void item_stats_reset(struct leveldb_engine *engine) {
...
static inline size_t ITEM_ntotal(struct leveldb_engine *engine,
...
/*@null@*/
hash_item *do_item_alloc(struct leveldb_engine *engine,
                         const void *key,
                         const size_t nkey,
                         const int flags,
...
static void item_link_q(struct leveldb_engine *engine, hash_item *it) { /* item is the new head */
    hash_item **head, **tail;
...
int do_item_link(struct leveldb_engine *engine, hash_item *it) {
...
void do_item_unlink(struct leveldb_engine *engine, hash_item *it) {
...
void do_item_release(struct leveldb_engine *engine, hash_item *it) {
...
void do_item_update(struct leveldb_engine *engine, hash_item *it) {
    rel_time_t current_time = engine->server.core->get_current_time();
...
int do_item_replace(struct leveldb_engine *engine,
                    hash_item *it, hash_item *new_it) {
...
static void do_item_stats(struct leveldb_engine *engine,
...
/*@null@*/
static void do_item_stats_sizes(struct leveldb_engine *engine,
                                ADD_STAT add_stats, const void *c) {
...
hash_item *do_item_get(struct leveldb_engine *engine,
                       const char *key, const size_t nkey) {
    rel_time_t current_time = engine->server.core->get_current_time();
 ...
 */
static ENGINE_ERROR_CODE do_store_item(struct leveldb_engine *engine,
 ...
 */
static ENGINE_ERROR_CODE do_add_delta(struct leveldb_engine *engine,
 ...
 */
hash_item *item_alloc(struct leveldb_engine *engine,
 ...
 */
hash_item *item_get(struct leveldb_engine *engine,
                    const void *key, const size_t nkey) {
    hash_item *it;
    pthread_mutex_lock(&engine->cache_lock);
 ...
 * needed.
 */
void item_release(struct leveldb_engine *engine, hash_item *item) {
    pthread_mutex_lock(&engine->cache_lock);
 ...
 */
void item_unlink(struct leveldb_engine *engine, hash_item *item) {
    pthread_mutex_lock(&engine->cache_lock);
    do_item_unlink(engine, item);
    pthread_mutex_unlock(&engine->cache_lock);
}

static ENGINE_ERROR_CODE do_arithmetic(struct leveldb_engine *engine,
                                       const void* cookie,
                                       const void* key,
                                       const int nkey,
...
ENGINE_ERROR_CODE arithmetic(struct leveldb_engine *engine,
                             const void* cookie,
                             const void* key,
                             const int nkey,
                             const bool increment,
 ...
 */
ENGINE_ERROR_CODE store_item(struct leveldb_engine *engine,
...
static hash_item *do_touch_item(struct leveldb_engine *engine,
                                     const void *key,
                                     uint16_t nkey,
                                     uint32_t exptime)
{
...
hash_item *touch_item(struct leveldb_engine *engine,
                           const void *key,
                           uint16_t nkey,
                           uint32_t exptime)
{
 ...
 */
void item_flush_expired(struct leveldb_engine *engine, time_t when) {
    int i;
    hash_item *iter, *next;
 ...
 */
char *item_cachedump(struct leveldb_engine *engine,
...
void item_stats(struct leveldb_engine *engine,
...
void item_stats_sizes(struct leveldb_engine *engine,
                      ADD_STAT add_stat, const void *cookie)
{
    pthread_mutex_lock(&engine->cache_lock);
...
static void do_item_link_cursor(struct leveldb_engine *engine,
...
typedef ENGINE_ERROR_CODE (*ITERFUNC)(struct leveldb_engine *engine,
                                      hash_item *item, void *cookie);

static bool do_item_walk_cursor(struct leveldb_engine *engine,
                                hash_item *cursor,
                                int steplength,
                                ITERFUNC itemfunc,
                                void* itemdata,
...
static ENGINE_ERROR_CODE item_scrub(struct leveldb_engine *engine,
                                    hash_item *item,
                                    void *cookie) {
    (void)cookie;
...
static void item_scrub_class(struct leveldb_engine *engine,
                             hash_item *cursor) {
...
{
    struct leveldb_engine *engine = arg;
...
bool item_start_scrub(struct leveldb_engine *engine)
{
    bool ret = false;
    pthread_mutex_lock(&engine->scrubber.lock);
...
static ENGINE_ERROR_CODE item_tap_iterfunc(struct leveldb_engine *engine,
                                    hash_item *item,
                                    void *cookie) {
...
static tap_event_t do_item_tap_walker(struct leveldb_engine *engine,
...
{
    tap_event_t ret;
    struct leveldb_engine *engine = (struct leveldb_engine*)handle;
    pthread_mutex_lock(&engine->cache_lock);
...
bool initialize_item_tap_walker(struct leveldb_engine *engine,
                                const void* cookie)
{
