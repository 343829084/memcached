...
static void log_free_buffer(void* p);
...
log_level_t log_level = LEVELDB_ENGINE_LOG_INFO;
...
log_set_debug_level(log_level_t level)
{
    ...
    if (level < LEVELDB_ENGINE_LOG_ERROR) level = LEVELDB_ENGINE_LOG_ERROR;
    if (level > LEVELDB_ENGINE_LOG_DEBUG) level = LEVELDB_ENGINE_LOG_DEBUG;

    log_level = level;
}

#ifdef LEVELDB_ENGINE_LOG_TEST
...
{
	pthread_t thread;
	FILE *file = fopen("leveldb-engine.log", "w+");
	log_set_stream(file);
