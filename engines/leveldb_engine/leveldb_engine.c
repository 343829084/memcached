 ...
 * =============================================================================
 *
 *       Filename:  leveldb_engine.c
 *
 *    Description:  leveldb engine implementation based on the default engine.
 *
...
static const engine_info* leveldb_get_info(ENGINE_HANDLE* handle);
static ENGINE_ERROR_CODE leveldb_initialize(ENGINE_HANDLE* handle,
                                            const char* config_str);
static void leveldb_destroy(ENGINE_HANDLE* handle,
                            const bool force);
static ENGINE_ERROR_CODE leveldb_item_allocate(ENGINE_HANDLE* handle,
                                               const void* cookie,
                                               item **item,
                                               const void* key,
                                               const size_t nkey,
...
static ENGINE_ERROR_CODE leveldb_item_delete(ENGINE_HANDLE* handle,
                                             const void* cookie,
                                             const void* key,
                                             const size_t nkey,
                                             uint64_t cas,
...
static void leveldb_item_release(ENGINE_HANDLE* handle, const void *cookie,
                                 item* item);
static ENGINE_ERROR_CODE leveldb_get(ENGINE_HANDLE* handle,
                                     const void* cookie,
                                     item** item,
                                     const void* key,
                                     const int nkey,
                                     uint16_t vbucket);
static ENGINE_ERROR_CODE leveldb_get_stats(ENGINE_HANDLE* handle,
                  const void *cookie,
                  const char *stat_key,
                  int nkey,
                  ADD_STAT add_stat);
static void leveldb_reset_stats(ENGINE_HANDLE* handle, const void *cookie);
static ENGINE_ERROR_CODE leveldb_store(ENGINE_HANDLE* handle,
                                       const void *cookie,
                                       item* item,
                                       uint64_t *cas,
                                       ENGINE_STORE_OPERATION operation,
                                       uint16_t vbucket);
static ENGINE_ERROR_CODE leveldb_arithmetic(ENGINE_HANDLE* handle,
                                            const void* cookie,
                                            const void* key,
                                            const int nkey,
                                            ...
                                            uint64_t *result,
                                            uint16_t vbucket);
static ENGINE_ERROR_CODE leveldb_flush(ENGINE_HANDLE* handle,
                                       const void* cookie, time_t when);
static ENGINE_ERROR_CODE initalize_configuration(struct leveldb_engine *se,
                                                 const char *cfg_str);
static ENGINE_ERROR_CODE leveldb_unknown_command(ENGINE_HANDLE* handle,
                                                 const void* cookie,
                                                 protocol_binary_request_header *request,
                                                 ADD_RESPONSE response);

static ENGINE_ERROR_CODE leveldb_tap_notify(ENGINE_HANDLE* handle,
                                            const void *cookie,
...
static TAP_ITERATOR leveldb_get_tap_iterator(ENGINE_HANDLE* handle,
                                             const void* cookie,
                                             const void* client,
                                             size_t nclient,
                                             uint32_t flags,
...
static void leveldb_handle_disconnect(const void *cookie,
...
static void set_vbucket_state(struct leveldb_engine *e,
...
static vbucket_state_t get_vbucket_state(struct leveldb_engine *e,
                                         uint16_t vbid) {
    union vbucket_info_adapter vi;
    vi.c = e->vbucket_infos[vbid];
    return vi.v.state;
...
static bool handled_vbucket(struct leveldb_engine *e, uint16_t vbid) {
...
ENGINE_ERROR_CODE create_instance(uint64_t interface,
   ...
   struct leveldb_engine *engine = malloc(sizeof(*engine));
   if (engine == NULL) {
      return ENGINE_ENOMEM;
   }

   struct leveldb_engine leveldb_engine = {
      .engine = {
         .interface = {
            .interface = 1
         },
         .get_info = leveldb_get_info,
         .initialize = leveldb_initialize,
         .destroy = leveldb_destroy,
         .allocate = leveldb_item_allocate,
         .remove = leveldb_item_delete,
         .release = leveldb_item_release,
         .get = leveldb_get,
         .get_stats = leveldb_get_stats,
         .reset_stats = leveldb_reset_stats,
         .store = leveldb_store,
         .arithmetic = leveldb_arithmetic,
         .flush = leveldb_flush,
         .unknown_command = leveldb_unknown_command,
         .tap_notify = leveldb_tap_notify,
         .get_tap_iterator = leveldb_get_tap_iterator,
   ...
   *engine = leveldb_engine;
   engine->tap_connections.clients = calloc(leveldb_engine.tap_connections.size, sizeof(void*));
...
static inline struct leveldb_engine* get_handle(ENGINE_HANDLE* handle) {
   return (struct leveldb_engine*)handle;
}
...
static ENGINE_ERROR_CODE leveldb_initialize(ENGINE_HANDLE* handle,
                                            const char* config_str) {
   struct leveldb_engine* se = get_handle(handle);
   ...
   se->server.callback->register_callback(handle, ON_DISCONNECT, leveldb_handle_disconnect, handle);

   return ENGINE_SUCCESS;
}

static void leveldb_destroy(ENGINE_HANDLE* handle, const bool force) {
   (void) force;
   struct leveldb_engine* se = get_handle(handle);
...
static ENGINE_ERROR_CODE leveldb_item_allocate(ENGINE_HANDLE* handle,
                                               ...
                                               const int flags,
                                               const rel_time_t exptime) {
   struct leveldb_engine* engine = get_handle(handle);
...
static void leveldb_item_release(ENGINE_HANDLE* handle,
                                 const void *cookie,
                                 item* item) {
...
static void stats_vbucket(struct leveldb_engine *e,
                          ADD_STAT add_stat,
                          const void *cookie) {
...
static bool set_vbucket(struct leveldb_engine *e,
                        const void* cookie,
...
static bool get_vbucket(struct leveldb_engine *e,
                        const void* cookie,
...
static bool rm_vbucket(struct leveldb_engine *e,
                       const void *cookie,
...
static bool scrub_cmd(struct leveldb_engine *e,
                      const void *cookie,
                      protocol_binary_request_header *request,
                      ADD_RESPONSE response) {
...
static bool touch(struct leveldb_engine *e, const void *cookie,
                  protocol_binary_request_header *request,
                  ADD_RESPONSE response) {
...
static ENGINE_ERROR_CODE leveldb_unknown_command(ENGINE_HANDLE* handle,
                                                 ...
                                                 ADD_RESPONSE response)
{
    struct leveldb_engine* e = get_handle(handle);
    bool sent;
...
static ENGINE_ERROR_CODE leveldb_tap_notify(ENGINE_HANDLE* handle,
    ...
    case TAP_FLUSH:
        return leveldb_flush(handle, cookie, 0);

    case TAP_DELETION:
        return leveldb_item_delete(handle, cookie, key, nkey, cas, vbucket);

    case TAP_MUTATION:
        it = engine->server.cookie->get_engine_specific(cookie);
        if (it == NULL) {
            ret = leveldb_item_allocate(handle, cookie, &it, key, nkey, ndata, flags, exptime);
            switch (ret) {
            case ENGINE_SUCCESS:
                break;
            case ENGINE_ENOMEM:
        ...
        ret = leveldb_store(handle, cookie, it, &cas, OPERATION_SET, vbucket);
...
static void leveldb_handle_disconnect(const void *cookie,
                                      ...
                                      const void *event_data,
                                      const void *cb_data) {
    struct leveldb_engine *engine = (struct leveldb_engine*)cb_data;
    pthread_mutex_lock(&engine->tap_connections.lock);
    int ii;
    for (ii = 0; ii < engine->tap_connections.size; ++ii) {
